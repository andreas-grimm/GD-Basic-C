...
Value* Function::evaluate() {
    ...
    switch (moToken.getType()) {
        ...
        case BasicTokenType::LOG: {
            return Log::execute(mpoFirstParam->evaluate());
        }
        ...
        case BasicTokenType::VAL: {
            return Val::execute(mpoFirstParam->evaluate());
        }
        default: {
...
string Function::content() {
    ...
    if (mpoFirstParam != NULL) {
        strReturn += mpoFirstParam->content();
    }

    if (mpoSecondParam != NULL) {
        strReturn += ", " + mpoSecondParam->content();
    }

    if (mpoThirdParam != NULL) {
        strReturn += ", " + mpoThirdParam->content();
    }
